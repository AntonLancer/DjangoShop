...
class CatalogView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if category_slug:
            current_category = get_object_or_404(Category, slug=category_slug) # Получаем категорию по слагу
            products = products.filter(category=current_category) # Получаем все товары по категории
...
class ProductDetailView(DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return TemplateResponse(request, self.template_name, context) # Если запрос не от htmx
